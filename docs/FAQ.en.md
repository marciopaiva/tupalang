...
Answer common questions about the project and language.

## Frequently Asked Questions

### 1) Is the project production ready?
Not yet. The v0.1 spec is complete, but the compiler is still under development.
...
In the [examples](../examples/README.md) folder.

### 5) What are `Safe<T, ...>`?
Types with compile-time proven constraints, e.g., `Safe<f64, !nan>`.

### 6) How do I run the CLI?
Use `cargo run -p tupa-cli -- <command>` and see [docs/GETTING_STARTED.en.md](GETTING_STARTED.en.md).

### 7) Is there a roadmap?
Yes: [docs/MVP_PLAN.md](MVP_PLAN.md) and [docs/ADOPTION_PLAN.md](ADOPTION_PLAN.md).

### 8) Can I propose changes to the spec?

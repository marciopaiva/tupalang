...
use tupa_runtime::{register_step, run_pipeline, evaluate_constraints};
...
fn step_validate_market_regime(input: Value) -> Result<Value, String> {
    let vol = input.get("input").unwrap().get("volatility").unwrap().as_f64().unwrap();
...
fn step_calculate_position_size(input: Value) -> Result<Value, String> {
    let balance = input.get("input").unwrap().get("balance").unwrap().as_f64().unwrap();
...
fn main() {
    ...
    tracing::info!(system = "viper_bot", version = "0.0.1", event = "startup", "Initializing Tupã Audit System");

    // 2. Register Steps
    register_step("viper::validate_market_regime", step_validate_market_regime);
    register_step("viper::calculate_position_size", step_calculate_position_size);

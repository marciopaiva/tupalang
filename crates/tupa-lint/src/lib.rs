...
impl LintWarning {
    pub fn message(&self) -> String {
        match self {
            ...
            LintWarning::MissingConstraintAnnotation(name) => {
                format!("Pipeline '{}' has no constraints", name)
            }
            LintWarning::ImpureInSensitiveContext(name) => {
                format!("Impure operation '{}' in sensitive context", name)
            }
            LintWarning::SnakeCase(kind, name) => {
                format!("{} '{}' should be snake_case", kind, name)
            }
            LintWarning::PascalCase(kind, name) => {
    ...
    fn declare(&mut self, name: String) {
        ...
        if let Some(scope) = self.scopes.last_mut() {
            if scope.contains_key(&name) {
                self.warnings
                    .push(LintWarning::ShadowedVariable(name.clone()));
            }
        ...
        match item {
            Item::Function(f) => {
                if !is_snake_case(&f.name) {
                    ctx.warnings.push(LintWarning::SnakeCase(
                        "Function".to_string(),
                        f.name.clone(),
                    ));
                }
            ...
            Item::Pipeline(p) => {
                if !is_pascal_case(&p.name) {
                    ctx.warnings.push(LintWarning::PascalCase(
                        "Pipeline".to_string(),
                        p.name.clone(),
                    ));
                }
                ...
                if p.constraints.is_empty() {
                    ctx.warnings
                        .push(LintWarning::MissingConstraintAnnotation(p.name.clone()));
                }
                ...
                let is_impure = p
                    .attrs
                    .iter()
                    .any(|a| a.name == "impure" || a.name == "@impure");
                if !is_impure {
    ...
    match stmt {
        Stmt::Let { name, expr, .. } => {
            if !is_snake_case(name) {
                ctx.warnings
                    .push(LintWarning::SnakeCase("Variable".to_string(), name.clone()));
            }
            lint_expr(expr, ctx);
        ...
        ExprKind::Call { callee, args } => {
            if let ExprKind::Ident(name) = &callee.kind {
                if ctx.in_sensitive_context && is_impure_func(name) {
                    ctx.warnings
                        .push(LintWarning::ImpureInSensitiveContext(name.clone()));
                }
            }
...
fn is_impure_func(name: &str) -> bool {
    matches!(
        name,
        "print" | "read" | "random" | "time" | "write" | "http_get"
    )
}
